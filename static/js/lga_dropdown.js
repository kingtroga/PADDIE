(function() {
    // Make sure django.jQuery is available before proceeding
    if (typeof django !== 'undefined' && django.jQuery) {
        var $ = django.jQuery;
        
        $(document).ready(function() {
            console.log("Script running with Django jQuery!");
    ...
    'Zamfara': [
        ...
        'Kaura Namoda', 'Maradun', 'Maru', 'Shinkafi', 'Talata Mafara', 'Chafe', 'Zurmi'
            ...
            // Get the state and LGA fields
            const stateField = $('#id_state');
            const lgaField = $('#id_lga');
            const isNationalField = $('#id_is_national');
            
            console.log("State field found:", stateField.length > 0);
            console.log("LGA field found:", lgaField.length > 0);
            console.log("Is national field found:", isNationalField.length > 0);
            ...
            function updateLGAOptions() {
                ...
                // Store the current LGA selection
                const currentLga = lgaField.val();
                
                // Clear existing options
                lgaField.empty();
                lgaField.append($('<option></option>').attr('value', '').text('-- Select LGA --'));
                
                // Add LGAs for the selected state
                if (selectedState && NIGERIAN_LGAS[selectedState]) {
                    NIGERIAN_LGAS[selectedState].forEach(function(lga) {
                        lgaField.append($('<option></option>').attr('value', lga).text(lga));
                    });
                    
                    console.log("Added LGAs for", selectedState);
                    
                    // Try to restore previous selection if valid
                    if (currentLga && NIGERIAN_LGAS[selectedState].includes(currentLga)) {
                        lgaField.val(currentLga);
                    }
                }
            }
            
            // Handle national group toggling
            function handleNationalToggle() {
                if (isNationalField.is(':checked')) {
                    console.log("Is national checked, disabling state and LGA");
                    stateField.val('').prop('disabled', true);
                    lgaField.val('').prop('disabled', true);
                } else {
                    console.log("Is national unchecked, enabling state and LGA");
                    stateField.prop('disabled', false);
                    lgaField.prop('disabled', false);
                    updateLGAOptions();
                }
            }
            ...
            updateLGAOptions();
            handleNationalToggle();
            
            // Add event listeners
            stateField.on('change', updateLGAOptions);
            isNationalField.on('change', handleNationalToggle);
        });
    } else {
        console.error("Django jQuery not available! Make sure you're using the Django admin interface.");
        
        // Try using window.jQuery as a fallback
        if (typeof jQuery !== 'undefined') {
            console.log("Using window.jQuery as fallback");
            jQuery(document).ready(function() {
                // Same code as above but using jQuery instead of $
                // This is a fallback in case django.jQuery isn't available
            });
        }
    }
})();